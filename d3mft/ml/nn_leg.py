...
def generate_data(meta=meta(1), # temperature
    ...
    n_input = 2 * n_l  # weak & strong coupling GF 
    n_output = n_l  # approximation to an exact GF
    
    skip = 1
    
    def get_data(prefix, n):
        # print(parent + prefix + meta + "_" + n + "_legendre.csv")        
        data = np.loadtxt(parent + prefix + meta + "_" + n + "_legendre.csv",
                          delimiter=",")[:, ::skip]
        return data

    n_train = len(train_files) * n_per_file
    n_test = len(val_files) * n_per_file
    ...
    def fill_with_data(X, Y, files):
        for i, n in enumerate(files):
            row_start = i*n_per_file
            row_end = (i+1)*n_per_file
            if n < 10 :
                str_n = "0"+str(n)
            else:
                str_n = str(n)
            X[row_start:row_end, :n_l] = get_data("G_PT", str_n)
            X[row_start:row_end, n_l:] = get_data("G_SC", str_n)
            Y[row_start:row_end, :] = get_data("G_ED_Q", str_n)            
...
def predict(model,
    ...
    for i, y in zip(samples, Y):
        plt.plot(leg, Y_test[i], 'o')
        plt.plot(leg, y, '--')
        plt.plot(leg, X_test[i, :n_l], '-.')
        plt.plot(leg, X_test[i, n_l:], '-.')
        plt.xlabel(r"$\tau$")
        plt.ylabel(r"$G_l$")
